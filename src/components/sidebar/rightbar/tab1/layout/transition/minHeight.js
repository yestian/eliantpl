...
class MaxHeight extends Component{
    ...
    componentDidMount(){
        let index=this.props.data.index;
        let node=index.siteData.data[index.sid];
        let res=ABC.thisCls(node,'min-height');
        this.setState({sid:index.sid})
    }
    componentDidUpdate(){
        let that=this;
    ...
    formSubmit(){
        let obj=this.props.form.getFieldsValue();
        if(!obj['min-height']){return false;}
        this.props.data.setValue('min-height',obj);
    }
    //打开弹窗判断
    openProp(e,states){
        if(!states){
        ...
        this.props.data.openPropWindow(e,'min-height')
    }
    ...
    formTranslate(index){
        ...
        //值的相互转换
        let res=ABC.transLateValue('min-height',obj,index,M);
        this.props.data.setValue2('min-height',res);
        //清除输入框的数据
        this.props.form.resetFields();
    }
    render(){
        ...
        if(res.hasOwnProperty('b') && res.b!=='none'){//是否存在b属性
            //min-height的值如果不是auto，把单位后缀和数字拆开
                ...
                <div className="left guidance-panel-wrapper">
                    <div className="guidance-panel-child">
                        <div className="kit-field pull-right">
                            <div className="label kit-label " title="最小高度" onClick={(e)=>{this.openProp(e,res.a)}}>
                            ...
                            <div className="kit-unit-box kit-text-input">
                                <Form id="form">
                                    <Input type="text"
                                        placeholder="0"
                                        className="input"
                                        {...getFieldProps('min-height', { initialValue: value })}
                                        autoComplete="off"
                                        onBlur={this.formSubmit}
                                    />
                                    <Input type="hidden" {...getFieldProps('unit', { initialValue: unit })}/>
                                </Form>
                                <span className="unit" onClick={()=>{data.openUnit(5)}}>{unit}</span>

                                {data.right.layout.openUnit===5?<Unit onClick={this.formTranslate} unit={unit}/>:''}
