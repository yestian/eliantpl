...
class Margin extends Component{
    componentDidMount(){
        $('.handle').draggable({
            ...
            iframeFix: true,
            scroll: true,
            helper:function(){return $('#ddd')},
            revert: "invalid",
            drag:function(event,ui){
                console.log(ui.position.top);
            }
        });
        $('.drag-ghost.row').keyup(function(){
            
        });
    }
    render(){
        // let right=this.props.right;
        let {showNodeMargin}=this.props;
        return(
            <div className="margin">
                <div id="ddd" className="handle sprite-mid top guidance-panel-wrapper" style={{opacity: 0}}><div className="guidance-panel-child" /></div>
                ...
                <div className="handle sprite-mid top" title="修改上边距" style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}
                    ...
                    onMouseDown={()=>{NodeMarginMouseDown(0)}}
                    onMouseUp={()=>{NodeMarginMouseDown(0)}}
                ...
                <div className="handle sprite-mid right vert" title="修改右边距" style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}><svg width={9} height={16} viewBox="0 0 9 16" className="bem-Svg " style={{display: 'block', transform: 'translate(0px, 0px)'}}
                    onMouseEnter={()=>{showNodeMargin(1)}}
                    onMouseLeave={()=>{showNodeMargin(0)}}><path d="M3 3h1v10H3zM1 1h1v14H1zm4 4h1v6H5zm2 2h1v2H7z" fill="currentColor" /></svg></div>
                ...
                <div className="handle sprite-mid left vert" title="修改左边距" style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}
                    onMouseEnter={()=>{showNodeMargin(1)}}
                    onMouseLeave={()=>{showNodeMargin(0)}}><svg width={9} height={16} viewBox="0 0 9 16" className="bem-Svg " style={{display: 'block', transform: 'translate(0px, 0px)'}}><path d="M5 3h1v10H5zm2-2h1v14H7zM3 5h1v6H3zM1 7h1v2H1z" fill="currentColor" /></svg></div>
...
{/* //需求
//margin,padding工具栏的，每次加1   click
//margin ,padding 鼠标按下  提示信息   mousedown
//margin ,padding 的鼠标悬浮 hover
// */}
