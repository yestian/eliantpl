...
class AddEleLayout extends Component{
    ...
    // 3.方法参数
     render(){
         ...
         let extend=this.props.ico.ico_event.addSub1Expand;//是否展开模块
         ...
         return(
             <div style={{borderBottom: 0, marginBottom: extend?0:'-1px'}}>
                 <div style={data.s2}>
                 <div style={data.s3}  onClick={()=>{this.props.addSubpageToggle('ADD_SUB1_TOGGLE')}}>
                     <div style={data.s5}>{extend?data.arrowD:data.arrowR}</div>
                     <div style={data.s4}>布局</div>
                 </div>
                 {/* 展开和收起，收起的时候去掉下面 */}
                 {extend?
                     (<div columncount={3} style={data.s6}>
                         {/* 所有的鼠标事件都绑定在data里面 */}
                         <div data-item-id={1} flexbasis={flex} style={s1}
                            onMouseEnter={(e)=>{itemMouseEnter(e)}}
                            onMouseLeave={itemMouseLeave}
                            onMouseUp={(e)=>{itemMouseUp(e)}}>
                            <Ele id={1} data={this.props} showHover={1}/>
                        </div>
                        <div data-item-id={2} flexbasis={flex} style={s1}
                            onMouseEnter={(e)=>{itemMouseEnter(e)}}
                            onMouseLeave={itemMouseLeave}
                            onMouseUp={(e)=>{itemMouseUp(e)}}>
                            <Ele id={2} data={this.props} showHover={1}/>
                        </div>
                        <div data-item-id={3} flexbasis={flex} style={s1}
                            onMouseEnter={(e)=>{itemMouseEnter(e)}}
                            onMouseLeave={itemMouseLeave}
                            onMouseUp={(e)=>{itemMouseUp(e)}}>
                            <Ele id={3} data={this.props} showHover={1}/>
                        </div>
                     </div>)
                 :null}
                 </div>
