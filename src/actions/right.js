...
//打开属性的弹窗
export function openPropWindow(e,prop){
    return function(dispatch){
        if(e===0){
            dispatch({type:OPEN_PROP_WINDOW,openPropWindow:0});
        }else{
            let openPropWindow={
                left:$('.right-sidebar').offset().left+1,
                top:$(e.target).offset().top-90,
                prop:prop
            }
            dispatch({type:OPEN_PROP_WINDOW,openPropWindow});
        }
    }
}
